...
from contribution.van import Attention
from pytorchcvaa.network.resnet import resnet18

#65.354000, 66.880000, 67.10(在block1之前加attn)，66.893(把block1替换为attn，测试66.804) 68.106(在block1之后加attn)
#67.188(DW-5,2;DWD-7,1,9,3)   68.106 68.120  67.192
...
class direct_dataset(Dataset):
    def __init__(self, logger, dataset):
        ...
        for i in range(1, 5):
            # data!
            path = "/luoyan/IntraQ-master/data_generate/result/myresnet18refined_gaussian" + str(i) + ".pickle"
            # path = "/luoyan/IntraQ-master/data_generate/cifarresult/myresnet20_cifar10labels_list" + str(i) + ".pickle"
            ...
            # label!
            path = "/luoyan/IntraQ-master/data_generate/result/myresnet18labels_list" + str(i) + ".pickle"
            # path = "/luoyan/IntraQ-master/data_generate/cifarresult/myresnet20_cifar10labels_list" + str(i) + ".pickle"
            self.logger.info(path)
            with open(path, "rb") as fp:  # Pickling
        ...
        #
        # # data!
        # path = "data_generate/result/resnet18refined_gaussian1.pickle"
        # self.logger.info(path)
        # with open(path, "rb") as fp:  # Pickling
        #     gaussian_data = pickle.load(fp)
        # # import IPython
        ...
        #
        # # label!
        # path = "data_generate/result/resnet18labels_list1.pickle"
        # self.logger.info(path)
        # with open(path, "rb") as fp:  # Pickling
        #     labels_list = pickle.load(fp)
        # if self.tmp_label is None:
...
class ExperimentDesign:
    def __init__(self, generator=None, options=None, conf_path=None, args=None):
        ...
        shutil.copyfile(conf_path, os.path.join(self.settings.save_path, 'imagenet.hocon'))     # imagenet,  cifar10_resnet20, cifar100_resnet20.hocon
        shutil.copyfile('/luoyan/IntraQ-master/mytrain.py', os.path.join(self.settings.save_path, 'mytrain.py'))
        shutil.copyfile('/luoyan/IntraQ-master/mytrainer.py', os.path.join(self.settings.save_path, 'mytrainer.py'))
    ...
    def _set_dataloader(self):
        ...
        self.test_loader = data_loader.getloader()

    def _set_model(self):
        if self.settings.dataset in ["cifar100", "cifar10"]:
            # self.test_input = Variable(torch.randn(1, 3, 32, 32).cuda())
            # self.model = ptcv_get_model(self.args.model_name, pretrained=False)
            # self.model_teacher = ptcv_get_model(self.args.model_name, pretrained=False)

            # state_dict = torch.load('/luoyan/IntraQ-master/pytorchcvaa/pretrained/resnet20_cifar100-2964-a5322afe.pth')      
            # #resnet20_cifar10-0597-9b0024ac.pth  resnet20_cifar100-2964-a5322afe.pth
            # self.model.load_state_dict(state_dict)
            # self.model_teacher.load_state_dict(state_dict)
            # self.model_teacher.eval()

            self.test_input = Variable(torch.randn(1, 3, 32, 32).cuda())
            self.model = resnet18(num_classes=self.settings.nClasses)
            self.model_teacher = resnet18(num_classes=self.settings.nClasses)

            state_dict = torch.load('/luoyan/IntraQ-master/pytorchcvaa/network/resnet18-f37072fd.pth')
            self.model.load_state_dict(state_dict)
            self.model_teacher.load_state_dict(state_dict)
            self.model_teacher.eval()
            ...
            # state_dict = torch.load('/luoyan/IntraQ-master/pytorchcvaa/pretrained/resnet18-0896-77a56f15.pth')      #resnet18-0982-0126861b.pth, 66.364   resnet18-0896-77a56f15.pth, 67.188
            # self.model.load_state_dict(state_dict)
            # self.model_teacher.load_state_dict(state_dict)
            # self.model_teacher.eval()
...
def main():
    ...
    parser.add_argument('--conf_path', type=str, metavar='conf_path', default="/luoyan/IntraQ-master/imagenet.hocon",
                        help='/luoyan/IntraQ-master/imagenet.hocon  | /luoyan/IntraQ-master/cifar10_resnet20.hocon  | /luoyan/IntraQ-master/cifar100_resnet20')
    parser.add_argument('--id', type=int, metavar='experiment_id', default='01',
                        help='Experiment ID')
    parser.add_argument('--model_name', type=str, metavar='model_name', default='resnet18',
                        help='resnet18  | resnet20_cifar10')
