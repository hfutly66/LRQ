...
from comeontrainer import Trainer
import shutil
...
#67.188(DW-5,2;DWD-7,1,9,3)   68.106 68.120
...
class direct_dataset(Dataset):
    def __init__(self, logger, dataset):
        ...
        for i in range(1, 5):
            # data!
            path = "/luoyan/IntraQ-master/data_generate/result/my_resnet18refined_gaussian" + str(i) + ".pickle"
            ...
            # label!
            path = "/luoyan/IntraQ-master/data_generate/result/my_resnet18labels_list" + str(i) + ".pickle"
            self.logger.info(path)
            with open(path, "rb") as fp:  # Pickling
...
class ExperimentDesign:
    def __init__(self, generator=None, options=None, conf_path=None, args=None):
        ...
        shutil.copyfile(conf_path, os.path.join(self.settings.save_path, 'imagenet.hocon'))
        shutil.copyfile('/luoyan/IntraQ-master/comeon.py', os.path.join(self.settings.save_path, 'comeon.py'))
        shutil.copyfile('/luoyan/IntraQ-master/comeontrainer.py', os.path.join(self.settings.save_path, 'comeontrainer.py'))
    ...
    def _set_model(self):
        if self.settings.dataset in ["cifar100", "cifar10"]:
            ...
            state_dict = torch.load('/luoyan/IntraQ-master/pytorchcvaa/pretrained/mobilenet_w1-0895-7e1d739f.pth')      
            #resnet18-0896-77a56f15.pth, mobilenet_w1-0895-7e1d739f, mobilenetv2_w1-0887-13a021bc
...
def main():
    ...
    parser.add_argument('--id', type=int, metavar='experiment_id', default='01',
                        help='Experiment ID')
    parser.add_argument('--model_name', type=str, metavar='model_name', default='mobilenet_w1',
                        help='resnet18, mobilenet_w1, mobilenetv2_w1')
