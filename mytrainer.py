...
import numpy as np
import torch
from contribution.dkd import DKD
from contribution.zaq import SCRM
...
class Trainer(object):
	...
	def __init__(self, model, model_teacher, generator, lr_master_S, lr_master_G,
		...
		scrm = SCRM().cuda()
		self.scrm = utils.data_parallel(
			scrm, self.settings.nGPU, self.settings.GPU)
	...
	def forward(self, images, teacher_outputs, labels=None):
		...
		dkdloss = DKD().cuda()
		loss = dkdloss(output, labels, teacher_outputs)
	...
	def train(self, epoch, direct_dataload=None):
		...
		for i in range(iters):
			...
			if epoch < 4:
				...
				self.var_list.clear()
				output_teacher_batch, feature_t = self.model_teacher(images)
				output, feature_s, loss_S = self.forward(images.detach(), output_teacher_batch.detach(), labels)
				inter_loss = self.scrm(feature_s, feature_t)
				...
				# loss of Generator
				loss_G = loss_one_hot + 0.1 * BNS_loss + 0.5*inter_loss
				...
				# IPython.embed()
				with torch.no_grad():
					output_teacher_batch, feature_t = self.model_teacher(images.detach())
				output, feature_s, loss_STU = self.forward(images.detach(), output_teacher_batch.detach(), labels)
				inter_loss = self.scrm(feature_s, feature_t)
				loss_S = loss_STU + 0.5*inter_loss
		...
		if epoch < 4:
			self.logger.info(
				"[Epoch %d/%d] [Batch %d/%d] [acc: %.4f%%] [G loss: %f] [One-hot loss: %f] [BNS_loss:%f] [S loss: %f] [inter_loss:%f]"
				% (epoch + 1, self.settings.nEpochs, i + 1, iters, 100 * fp_acc.avg, loss_G.item(),
				   loss_one_hot.item(), BNS_loss.item(),
				   loss_S.item(), inter_loss.item())
			)
		else:
			self.logger.info(
		...
		with torch.no_grad():
			for i, (images, labels) in enumerate(self.test_loader):
				...
				if self.settings.tenCrop:
					...
					output, _ = self.model_teacher(images)
